...
// Every scan is written to SQLite first (see insertScan.js) and marked
// synced = 0. This uploads any not-yet-synced rows for the current user to
// Firestore, then flips synced = 1 in SQLite so they aren't re-uploaded.
export const syncPendingScans = async () => {
  ...
  for (const scan of pending) {
    try {
      ...
      // Leave synced = 0 so this scan is retried on the next sync pass.
      console.warn(`⚠️ Failed to sync scan ${scan.scan_uuid}`, err);
    }
  }
};

// Calls syncPendingScans() once now, then again every time the device
// regains connectivity. Returns an unsubscribe function.
export const watchAndSyncScans = () => {
  syncPendingScans().catch((err) =>
    console.warn('⚠️ Initial scan sync failed', err)
  );
  ...
  const unsubscribe = NetInfo.addEventListener((state) => {
    ...
    if (isOnline && wasConnected === false) {
      syncPendingScans().catch((err) =>
        console.warn('⚠️ Reconnect scan sync failed', err)
      );
    }
